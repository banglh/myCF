...
//		bsgd_training_rmse(logFile, iteration, gcontext);
		run_validation(pvalidation_engine, gcontext);
//		bsgd_run_validation(logFile, pvalidation_engine, gcontext);
	}
...
void output_sgd_result(std::string filename) {
	...
	logstream(LOG_INFO) << "SGD output files (in matrix market format): "
			<< filename << "-baseline_P.mm" << ", " << filename + "-baseline_Q.mm " << std::endl;
}
