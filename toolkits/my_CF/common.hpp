...
void remove_cached_files() {
	...
	assert(training != "");
	rc = system(
			(std::string("rm -fR ") + training + std::string(".*")).c_str());
	assert(!rc);
	rc =
			system(
					(std::string("rm -fR ") + training
							+ std::string("_degs.bin")).c_str());
	assert(!rc);
	if (validation != "") {
		rc =
				system(
						(std::string("rm -fR ") + validation + std::string(".*")).c_str());
		assert(!rc);
	}
...
void parse_command_line_args() {
	...
	kfold_cross_validation_index = get_option_int(
			"kfold_cross_validation_index", kfold_cross_validation_index);
	if (kfold_cross_validation_index > 0) {
		if (kfold_cross_validation_index >= kfold_cross_validation)
			logstream(LOG_FATAL)
					<< "kfold_cross_validation index should be between 0 to kfold_cross_validation-1 parameter"
					<< std::endl;
	}
	if (kfold_cross_validation != 0) {
		logstream(LOG_WARNING) << "Activating kfold cross vlidation with K="
				<< kfold_cross_validation << std::endl;
		if (validation != "" || test != "")
			logstream(LOG_FATAL)
					<< "Using cross validation, validation file (--validation) and test file (--test) should be empty/"
					<< std::endl;
		//removing tmp file (if present)
		int rc = system(("rm -fR " + training + "_kfold_tmp_file").c_str());
		if (rc != 0)
			logstream(LOG_FATAL)
					<< "Failed to delete temp file. Please check permissions."
					<< std::endl;
		//linking training to validation
		rc =
				system(
						("ln -s " + training + " " + training
								+ "_kfold_tmp_file").c_str());
		if (rc != 0)
			logstream(LOG_FATAL)
					<< "Failed to link temp file. Please check permissions."
					<< std::endl;
...
void print_copyright() {
	logstream(LOG_WARNING)
			<< "GraphChi Collaborative filtering library is written by Danny Bickson (c). Send any "
					" comments or bug reports to danny.bickson@gmail.com "
			<< std::endl;
}
...
void print_config() {
	...
	std::cout << "[number_of_threads] => [" << number_of_omp_threads() << "]"
			<< std::endl;
	std::cout << "[membudget_Mb] => [" << get_option_int("membudget_mb") << "]"
			<< std::endl;
}

void bsgd_print_config() {
	std::cout << "****** BSGD experiment *******" << std::endl;
...
// randomly init 1	(rmin < rate < rmax)
template<typename T>
void init_random_bounded(T& latent_factors_inmem, bool randomize = true, double scale = 1.0) {
	std::cout<<"Starting random initialization ..." << std::endl;
	uint size = M + N;
	assert(size > 0);
	...
	// randomly initialize P and Q ([0,1])
	mat P = mat::Random(M, D);		// [-1,1]
	mat Q = mat::Random(D, N);		// [-1,1]
	P = (P + mat::Ones(M, D)) / 2;				// [0,1]
	Q = (Q + mat::Ones(D, N)) / 2;				// [0,1]
	...
	// recalculate P, Q
	double tempt = sqrt((maxval - 1) / maxE1);
	P = P * tempt;
	Q = Q * tempt;
	set_col(P, 0, ones(M));
	set_row(Q, 0, ones(N));

	// set values for latent_factors_inmem
	for (int i = 0; i < (int) M; i++) {
		latent_factors_inmem[i].pvec = get_row(P, i);
	}
	for (int i = M; i < (int) size; i++) {
		latent_factors_inmem[i].pvec = get_col(Q, i - M);
	}
}

template<typename T>
void init_baseline(T& latent_factors_inmem) {
	int size = M + N;
	assert(size > 0);
...
#pragma omp parallel for
	for (int i = 0; i < (int) size; i++) {
		for (int j = 0; j < D; j++)
			latent_factors_inmem[i].pvec[j] = 0.0;
	}
}
