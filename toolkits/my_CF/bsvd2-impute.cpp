 ...
 * @version 1.0
 *
 * bounded-SVD 2 with imputed missing ratings
 */
...
// some params for bsvd2_impute
double r_impute = minval;
		...
		if (vertex.num_outedges() > 0) {
			...
			for (int i = M; i < M + N; i++) {
				if (ratedItems.count(i) == 0) {
					...
					double temp = muy * lambda * (err - alpha * expTerm);
...
void print_config_to_file(std::ofstream & f) {
	f << "****** bounded-SVD 2 + imputed missing ratings *****"
			<< std::endl;
...
void bsgd2_impute_print_config() {
	std::cout
			<< "****** bounded-SVD 2 + imputed missing ratings *******"
			<< std::endl;
	...
	std::cout << "	+ r_impute = " << r_impute << std::endl;
	std::cout << "	+ muy = " << muy << std::endl;
	std::cout << "	+ step_dec = " << step_dec << std::endl;
	std::cout << "	+ max iterations = " << niters << std::endl;
	std::cout << "	+ halt_on_rmse_increase = " << halt_on_rmse_increase
			<< std::endl;
	std::cout << "	+ halt_on_minor_improvement = " << halt_on_minor_improvement
			<< std::endl;
}
int main(int argc, const char ** argv) {
	...
	r_impute = get_option_float("r_impute", 1);
...
//	}
	...
	if (logMode == 1) {
		...
		fn << basePath.str() << now << "_" << dataset << "_BSGD2-impute" << "_"
	...
	if (logMode == 1) {
		...
		baseFn << basePath.str() << now << "_" << dataset << "_BSGD2-impute" << "_"
				<< initType << "_RMSE_K" << D << "_Experiment" << experiment
				<< "_Run" << run;
