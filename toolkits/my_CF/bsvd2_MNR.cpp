 ...
 * @version 1.0
 *
 * bounded-SVD 2 with heuristic distribution of missing ratings
 */
...
// some params for bsvd2_MNR
double a_slope = 1.0;
double b_intercept = -1;
	...
	void update(graphchi_vertex<VertexDataType, EdgeDataType> &vertex,
		...
		if (vertex.num_outedges() > 0) {
			...
			for (int e = 0; e < vertex.num_edges(); e++) {
				...
				vec userMod = temp * item.pvec;
				vec itemMod = temp * user.pvec;
			...
			for (int i = M; i < M + N; i++) {
				if (ratedItems.count(i) == 0) {
					...
					double temp = muy * lambda * (alpha * expTerm + a_slope);
...
void print_config_to_file(std::ofstream & f) {
	f << "****** bounded-SVD 2 + heuristic distribution experiment *******" << std::endl;
	...
	f << "	+ a_slope = " << a_slope << std::endl;
	f << "	+ b_intercept = " << b_intercept << std::endl;
	f << "	+ muy = " << muy << std::endl;
	f << "	+ step_dec = " << step_dec << std::endl;
	f << "	+ max iterations = " << niters << std::endl;
	f << "	+ halt_on_rmse_increase = " << halt_on_rmse_increase << std::endl;
	f << "	+ halt_on_minor_improvement = " << halt_on_minor_improvement << std::endl;
}

void bsgd2_MNR_print_config() {
	std::cout << "****** bounded-SVD 2 + heuristic distribution experiment *******" << std::endl;
	...
	std::cout << "	+ a_slope = " << a_slope << std::endl;
	std::cout << "	+ b_intercept = " << b_intercept << std::endl;
...
int main(int argc, const char ** argv) {
	...
	a_slope = get_option_float("a_slope", 1);
	b_intercept = get_option_float("b_intercept", -1);
...
//	}
	...
	if (logMode == 1) {
		...
		fn << basePath.str() << now << "_" << dataset << "_BSGD2-MNR"
				<< "_" << initType << "_RMSE_K" << D << "_Exp" << experiment
				<< "_Run" << run << ".txt";
		fileName = fn.str();
	...
	if (logMode == 1) {
		...
		baseFn << basePath.str() << now << "_" << dataset << "_BSGD-2MNR"
				<< "_" << initType << "_RMSE_K" << D
				<< "_Experiment" << experiment << "_Run" << run;
